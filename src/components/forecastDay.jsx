...
const ForecastDay = ({
  ...
  const highClass =
    dayForecast.temp.max < minHighTemp || dayForecast.temp.max > maxTemp
      ? "high alert-day"
      : "high";
  ...
  if (dayNum === shipDays) {
    dayClass = "forecast-day arriving";
  }
  ...
  return (
    <div className={dayClass} key={dayNum}>
      ...
      <div className="temps">
        <div className={highClass}>{Math.round(dayForecast.temp.max)}&deg;</div>
        <div className={lowClass}>{Math.round(dayForecast.temp.min)}&deg;</div>
      </div>
    </div>
  );
};
