...
class App extends React.Component {
  ...
  render() {
    ...
    let filtered = [...forecasts].sort((a, b) =>
      a.dealer > b.dealer ? 1 : -1
    );

    if (filter) {
      filtered = filtered.filter((forecast) =>
        forecast.dealer.toLowerCase().includes(filter.toLowerCase())
      );
    }
    if (showAlertsOnly) {
      filtered = filtered.filter((forecast) =>
    ...
    let alertDealers = forecasts
      .sort((a, b) => (a.dealer > b.dealer ? 1 : -1))
      .filter((forecast) =>
        isWeatherAlert(
          forecast,
          minHighTemp,
    ...
    if (!showIntlDealers) {
      filtered = filtered.filter(
        (forecast) => forecast.country === "United States"
      );
      alertDealers = alertDealers.filter(
        (forecast) => forecast.country === "United States"
      );
    }
    ...
    return (
      <React.Fragment>
        ...
        >
          ...
          <path
            d="M28 0 L28 0 L28 0 L28 0 L28 0 L28 0 L28 0 C18 22 48 36 26 64 L64 64 L64 0 Z"
            fill="url(#grad)"
          />
        </svg> */}
          ...
          <section id="header">
            ...
            <div className="button-container">
              <Button
                ...
                style={
                  isLoading ? "btn btn-primary loading" : "btn btn-primary"
                }
